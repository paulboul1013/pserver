...
char *convert(char *src) {
    char *iter = src;
    char *result = malloc(sizeof(src)+1);
    char *it = result;
...
void* clientSocket(void *param){
    ...
    // 不斷接收 client 資料
    ssize_t n;
    while ((n = recv(info->sockfd, buf, sizeof(buf)-1, 0)) > 0) {
        buf[n] = '\0';
        // 收到 exit 指令就離開迴圈
        ...
        if (send(info->sockfd, conv, strlen(conv), 0) < 0) {
...
int main() 
{
    ...
    while(1) {
        ...
        pthread_detach(clients[client_index]);
